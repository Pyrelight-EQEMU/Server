	...
	if (chance > 0 || (IsOfClientBot() && GetClass() == WIZARD && GetLevel() >= RuleI(Spells, WizCritLevel))) {
		...
		if (zone->random.Roll(chance)) {
			...
			if (RuleR(Character, HeroicIntelligenceExtraCriticalDamage) > 0) {
				...
				if (IsPetOwnerClient()) {
					ratio *= 1 + ((1/3) * GetOwnerOrSelf()->GetHeroicINT() * RuleR(Character, HeroicIntelligenceExtraCriticalDamage));
				}
			}
		}
		...
		if (Critical) {
			...
			if (RuleI(Character, HeroicWisdomDamageReduction) > 0) {
				...
				if (target->IsClient() && target->GetHeroicSTA() > 0) {
					...
					damage_reduction_value = (2/3) * RuleI(Character, HeroicWisdomDamageReduction) * target->GetHeroicSTA();
				}
