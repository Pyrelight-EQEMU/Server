...
int64 Mob::GetActSpellDamage(uint16 spell_id, int64 value, Mob* target) {
	...
	if (chance > 0 || (IsOfClientBot() && GetClass() == WIZARD && GetLevel() >= RuleI(Spells, WizCritLevel)) ) {
		...
		if (GetClass() == MAGICIAN || GetClass() == DRUID || GetClass() == ENCHANTER || GetClass() == CLERIC) {
...
int64 Mob::GetActSpellHealing(uint16 spell_id, int64 value, Mob* target, bool from_buff_tic) {
	...
	if (RuleR(Character, Pyrelight_hWIS_HealPower) > 0) {
		...
		effective_hWIS = GetOwner() ? round(RuleR(Character, Pyrelight_HeroicPetMod) * GetOwner()->GetHeroicWIS()) : GetHeroicWIS();
	...
	if (GetClass() == SHAMAN || GetClass() == DRUID || GetClass() == CLERIC) {
		// Pyrelight Custom Code
		// Crit Chance based on HeroicCHA
		if ((IsClient() || (IsPet() && GetOwner() && IsPetOwnerClient()))) {
			if (RuleR(Custom,Pyrelight_Heroic_CritChance) > 0) {
				Mob* source = IsClient() ? this : GetOwner();
				critical_chance += std::min(static_cast<int>(floor(source->GetHeroicCHA() * RuleR(Custom,Pyrelight_Heroic_CritChance) / 10)), 25);
			}
		}
	}
