							   ...
							   (IsEffectInSpell(buffs[buffs_i].spellid, SE_Rune) ||
								...
								for (int effect : effectsToCheck) {
									...
									if (regen_amount > 0) {
										...
										if (regenerated) {
											if (IsClient()) {
												Message(Chat::Spells, "Your %s has regenerated by %i!", spells[buffs[buffs_i].spellid].name, regen_amount);
											} else if (IsPet() && GetOwner()) {
												GetOwner()->Message(Chat::Spells, "Your pet's %s has regenerated by %i!", spells[buffs[buffs_i].spellid].name, regen_amount);
											}
										}
									}
								}
