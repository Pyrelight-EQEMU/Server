	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch(effect)
		{
			...
			case SE_Stun:
			{
				...
				int channelchance = 30 + caster->GetSkill(EQ::skills::SkillChanneling) / 400.0f * 100;
						...
						(caster->IsNPC() && !RuleB(Spells, NPCIgnoreBaseImmunity))))))
				{
					...
					if (stun_resist <= 0 || zone->random.Int(0,99) >= stun_resist) {
						...
						Stun(effect_value);
					} else if (zone->random.Roll(channelchance) || caster->PL_DoHeroicChannelReroll(channelchance)) {
						caster->MessageString(Chat::Spells, REGAIN_AND_CONTINUE);
			...
			{
				if (buffslot > -1) {
					if (RuleR(Character, Pyrelight_Heroic_RuneBonus) > 0) {
						effect_value += PL_HeroicRuneBonus(effect_value);
					}
			...
			{
				if (buffslot > -1) {
					int32 rune_value = spells[spell_id].max_value[i];

					if (RuleR(Character, Pyrelight_Heroic_RuneBonus) > 0) {						
						rune_value += PL_HeroicRuneBonus(rune_value);
					}
				...
				{								
					...
					if (!spells[buffs[buffs_i].spellid].short_buff_box && (IsClient() || (IsPet() && IsPetOwnerClient()))) {
						...
						if (caster && client) {
							...
							// Regenerate Runes
							if (suspended && 
							   (IsEffectInSpell(buffs[buffs_i].spellid, SE_Rune) ||
							    IsEffectInSpell(buffs[buffs_i].spellid, SE_AbsorbMagicAtt))) {
								...
								for (int effect : effectsToCheck) {
									...
									if (regen_amount > 0) {
										...
										switch (effect) {
											case SE_Rune:
												rune = &buffs[buffs_i].melee_rune;
												break;
											case SE_AbsorbMagicAtt:
												rune = &buffs[buffs_i].magic_rune;
												break;
											default:
												break; // If the effect is neither, we exit the switch
										}
										...
										if (regenerated) {
											if (rune < max) {
												if (IsClient()) {
													Message(Chat::Spells, "Your %s has regenerated by %i points to %i!", spells[buffs[buffs_i].spellid].name, regen_amount, *rune);
												} else if (IsPet() && GetOwner()) {
													GetOwner()->Message(Chat::Spells, "Your pet's %s has regenerated by %i points to %i!", spells[buffs[buffs_i].spellid].name, regen_amount, *rune);
												}
											} else {
												if (IsClient()) {
													Message(Chat::Spells, "Your %s has regenerated by %i points to the maximum of %i!", spells[buffs[buffs_i].spellid].name, regen_amount, *rune);
												} else if (IsPet() && GetOwner()) {
													GetOwner()->Message(Chat::Spells, "Your pet's %s has regenerated by %i points to the maximum of %i!", spells[buffs[buffs_i].spellid].name, regen_amount, *rune);
												}
											}
										}
									}
