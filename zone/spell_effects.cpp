	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch(effect)
		{
			...
			case SE_NegateAttacks:
			{
				...
				if(buffslot >= 0) {
					if (RuleB(Spells, RuneUseHealAmt) && (IsClient() || IsBot())) {											
							buffs[buffslot].hit_number = static_cast<uint32>(std::ceil(effect_value * ((1 + itembonuses.HealAmt)/25))) ;
					} else {
...
void Mob::BuffProcess()
{
	...
	if (IsClient()) {
		...
		LogDebug("My Pet's Owner: [{}]", GetPet() ? (GetPet()->IsPetOwnerClient() ? "Pet Owner Is Client" : "Pet Owner Is Not Client") : "No Pet Found");
