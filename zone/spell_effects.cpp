									...
									if (regen_amount > 0) {
										...
										if (regenerated) {
											if (*rune < max) {
												if (IsClient()) {
													Message(Chat::Spells, 
															"Your %s has regenerated to %i points of protection!", 
															spells[buffs[buffs_i].spellid].name, 
															*rune);
												} else if (IsPet() && GetOwner()) {
													GetOwner()->Message(Chat::Spells, 
																		"Your pet's %s has regenerated to %i points of protection!", 
																		spells[buffs[buffs_i].spellid].name,
																		 *rune);
												}
											} else {
												if (IsClient()) {
													Message(Chat::Spells, 
															"Your %s has regenerated to its maximum %i points of protection!", 
															spells[buffs[buffs_i].spellid].name, 
															*rune);
												} else if (IsPet() && GetOwner()) {
													GetOwner()->Message(Chat::Spells, 
